...
import { Color, StrainMetadata, StrainPart } from '../models/straindata.model';
...
export async function buildRandomStrain(): Promise<Image> {
    ...
    const partsToCombine = [pot, head, body, face];
    const misc = Math.random() < 0.2 ? miscs[Math.floor(Math.random() * miscs.length)] : null;
    if (misc) {
        partsToCombine.push(misc);
    }
    return await buildStrain(partsToCombine, {
...
strainRoutes.get('/image/:dna', async (req, res) => {
    let dna = req.params.dna;
    const img = await getImageForDna(Number.parseInt(dna));
    res.contentType('image/png');
    const stream = serializeImage(img);
    stream.pipe(res);
});
